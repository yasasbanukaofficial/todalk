...
class _RecordingScreenState extends ConsumerState<RecordingScreen>
  ...
  void initState() {
    super.initState();
    _animController = AnimationController(vsync: this, duration: const Duration(milliseconds: 1600))..repeat();
    _init();
  }
  ...
  Future<void> _init() async {
    ...
    final user = ref.read(authProvider).user?.name;
  ...
  void _onVoiceEvent(VoiceEvent event) {
    ...
    switch (event.type) {
      ...
      case VoiceEventType.stateChange:
        setState(() { _currentState = event.data as String; _currentText = ''; });
        break;
      case VoiceEventType.transcript:
        final json = event.data as Map<String, dynamic>;
        final text = json['text'] as String? ?? '';
        if (text.isNotEmpty) setState(() => _currentText = text);
        break;
      ...
      case VoiceEventType.error:
        ...
        if (_currentState == 'connecting') setState(() => _currentState = 'error');
        break;
    }
  }
  ...
  void _onTaskCreated(Map<String, dynamic> taskData) {
    ...
    if (!_isSpeaking && mounted) Navigator.of(context).pop(true);
  }

  void _onTapDown() {
    if (_currentState == 'error') { _retry(); return; }
    if (_currentState != 'awaitingTask' && _currentState != 'awaitingDate' && _currentState != 'awaitingPriority') return;
  ...
  Future<void> _retry() async {
    setState(() => _currentState = 'connecting');
    _voice?.disconnect();
    _voice = await VoiceService.create(userName: ref.read(authProvider).user?.name);
    _voiceSub?.cancel();
    _voiceSub = _voice!.events.listen(_onVoiceEvent);
    await _voice!.connect();
  }
  ...
  Future<void> _startRecording() async {
    try {
      ...
      setState(() { _isListening = false; _isHolding = false; });
    }
  }
  ...
  Future<void> _playBufferedAudio() async {
    ...
    try {
      int totalSize = 0;
      for (final chunk in _audioBuffer) totalSize += chunk.length;
      final merged = Uint8List(totalSize);
      int offset = 0;
      for (final chunk in _audioBuffer) { merged.setRange(offset, offset + chunk.length, chunk); offset += chunk.length; }
    ...
    if (_taskSaved && mounted) Navigator.of(context).pop(true);
  }
  ...
  String _statusText() {
    ...
    if (_currentState == 'error') return 'TAP TO RETRY';
  ...
  String _promptText() {
    switch (_currentState) {
      case 'error': return 'Could not connect to server. Tap the circle to try again.';
  ...
  double get _blobScale => _isHolding ? 1.0 : _isAwaitingInput ? 0.92 : 1.0;
            ...
            return Stack(children: [
              SafeArea(child: Column(children: [
                ...
                Expanded(child: Center(child: GestureDetector(
                  ...
                  child: AnimatedScale(scale: scale, duration: const Duration(milliseconds: 200),
                    child: MorphingBlob(amplitude: amplitude, phase: phase, soundLevel: _soundLevel, size: 200)),
                ))),
                const SizedBox(height: 12),
                AnimatedSwitcher(duration: const Duration(milliseconds: 250),
                  child: _isHolding && _currentText.isNotEmpty
                    ? Padding(key: const ValueKey('transcript'), padding: const EdgeInsets.symmetric(horizontal: 40),
                        child: Text(_currentText, textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis,
                          style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w500, color: AppColors.white)))
                    : Padding(key: const ValueKey('prompt'), padding: const EdgeInsets.symmetric(horizontal: 40),
                        child: Text(_isHolding ? '' : _promptText(), textAlign: TextAlign.center,
                          style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w400, color: AppColors.textSecondary)))),
                const SizedBox(height: 16),
                SizedBox(height: 18, child: Text(_statusText(),
                  style: TextStyle(fontSize: 11, letterSpacing: 3, fontWeight: FontWeight.w500,
                    color: _isHolding ? AppColors.textSecondary : AppColors.textTertiary))),
                const Spacer(),
              ])),
              Positioned(top: MediaQuery.of(context).padding.top + 4, right: 8,
                child: GestureDetector(onTap: _cancel,
